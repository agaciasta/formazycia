...
async function loadDataAndGenerateJSON(url) {
  ...
  lines.forEach((line) => {
    if (line.startsWith("|")) {
      ...
      let exerciseNameForUrl = currentExercise.replace(/\s+/g, '_');
      exerciseNameForUrl = removeDiacritics(exerciseNameForUrl);
      jpgUrl = encodeURI(`../img/atlas_cwiczen/${exerciseNameForUrl}_1.jpg`);
      jpgUrl2 = encodeURI(`../img/atlas_cwiczen/${exerciseNameForUrl}_2.jpg`);
      steps = [];
    } else if (line.trim() !== "") {
